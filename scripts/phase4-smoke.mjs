...
function findBrowser() {
  ...
  throw new Error("No Chrome/Chromium executable found for the Phase 4 browser smoke test");
}
...
async function currentState(wsUrl) {
  return await evaluate(wsUrl, `(() => {
    ...
    return {
      ready: document.body.dataset.phase4 === 'ready',
      ...
      tetherDistance: number('#diag-tether-distance'),
      tetherLength: number('#diag-tether-length'),
      tetherTension: number('#diag-tether-tension'),
      tetherLoad: number('#diag-tether-load'),
      tetherRelease: document.querySelector('#diag-tether-release')?.textContent ?? '',
...
async function waitForReadyPage() {
  ...
  throw new Error(`Phase 4 app did not initialize. lastState=${JSON.stringify(lastState)} lastError=${lastError?.message ?? "none"}\n${browserOutput}`);
}
...
try {
  ...
  if (!response?.ok) throw new Error(`Vite preview failed to serve the Phase 4 build.\n${previewOutput}`);
  ...
  let working = await currentState(wsUrl);
  ...
  if (!completed || completed.lastCut !== "spine-panel" || completed.bodies !== 7 || completed.components !== 6) {
    throw new Error(`Live panel cut failed before tether proof. state=${JSON.stringify(completed ?? await currentState(wsUrl))}`);
  }

  await key(wsUrl, "keyDown", "t", "KeyT", 84);
  await sleep(180);
  const tetherStart = await currentState(wsUrl);
  if (!(tetherStart.tetherState === "attached" && tetherStart.tetherTarget === "panel" && Number.isFinite(tetherStart.tetherDistance))) {
    throw new Error(`Live tether did not acquire the detached panel. state=${JSON.stringify(tetherStart)}`);
  }
  await sleep(1200);
  const tetherPulled = await currentState(wsUrl);
  if (!(tetherPulled.tetherState === "attached" && tetherPulled.tetherTarget === "panel" && tetherPulled.tetherTension > 0 && tetherPulled.tetherLoad < 100 && tetherPulled.tetherDistance < tetherStart.tetherDistance - 0.08)) {
    throw new Error(`Live tether did not produce bounded pulling behavior. start=${JSON.stringify(tetherStart)} pulled=${JSON.stringify(tetherPulled)}`);
  }
  await key(wsUrl, "keyUp", "t", "KeyT", 84);
  await sleep(180);
  const released = await currentState(wsUrl);
  if (!(released.tetherState === "idle" && released.tetherTension === 0 && released.tetherRelease === "manual")) {
    throw new Error(`Live tether did not release cleanly. state=${JSON.stringify(released)}`);
  }

  await hold(wsUrl, "x", "KeyX", 88, 40);
  await sleep(450);
  const reset = await currentState(wsUrl);
  if (!(reset.bodies === 7 && reset.components === 6 && reset.connections === 6 && reset.severed === 0 && reset.tetherState === "idle" && reset.tetherTarget === "none" && Math.abs(reset.x) < 0.15 && Math.abs(reset.y) < 0.15 && Math.abs(reset.z - 14) < 0.2)) {
    throw new Error(`Live reset did not restore the Phase 4 baseline. state=${JSON.stringify(reset)}`);
  }

  console.log(`phase4 smoke: live panel cut + tether acquire/pull/release + exact reset passed in ${browserPath}; bodies=7; joints=6->5->6`);
} finally {
