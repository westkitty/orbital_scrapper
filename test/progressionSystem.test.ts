...
test("settlement credit, Clamp Dampers purchase, and next-run capability persist across reload", () => {
...
test("failed run accounting remains idempotent and preserves earned progression", () => {
  const storage = new MemoryStorage();
  const progression = new ProgressionSystem(storage);
  ...
  const diagnostics = new ProgressionSystem(storage).getDiagnostics();
...
test("corrupt or unsupported save data recovers to safe current defaults", () => {
  const malformed = new MemoryStorage();
  malformed.setItem(PROGRESSION_SAVE_KEY, "{not-json");
  ...
  future.setItem(PROGRESSION_SAVE_KEY, JSON.stringify({ version: 99, credits: 999999 }));
